  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-gotoap-bg">
      <header className="flex h-14 shrink-0 items-center gap-3 border-b border-gotoap-line bg-gotoap-panel px-4">
        ...
        <h1 className="text-base font-semibold text-gotoap-ink">Groups</h1>
      </header>
      <GroupsList onNewGroup={() => setComposeOpen(true)} />
      <CreateGroupDialog open={composeOpen} onClose={() => setComposeOpen(false)} onCreated={(id) => { setComposeOpen(false); router.push('/chat/' + encodeURIComponent(id)); }} />
    </div>
  );
}