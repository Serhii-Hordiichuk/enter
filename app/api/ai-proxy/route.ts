    ...
    for (;;) {
      ...
      while (boundary >= 0) {
        ...
        for (const line of frame.split('\n')) {
          ...
          } catch (error) { console.error('AI-проксі: пошкоджений фрагмент OpenAI:', error); }
        }
      ...
      while (boundary >= 0) {
        ...
        if (event.data) {
          try {
            ...
            if (token) yield token;
          } catch (error) { console.error('AI-проксі: пошкоджений фрагмент Anthropic:', error); }
        }
        boundary = buffer.indexOf('\n\n');
      }
    }
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (!body.messages || !Array.isArray(body.messages) || body.messages.length === 0 || !body.messages.every(isChatMessage)) {
      return NextResponse.json({ error: 'Поле messages має бути непорожнім масивом повідомлень чату' }, { status: 400 });
    }
    ...
    if (wantsAnthropic && !anthropicKey) return NextResponse.json({ error: 'ANTHROPIC_API_KEY не налаштовано на сервері' }, { status: 500 });
    if (!wantsAnthropic && !openaiKey) return NextResponse.json({ error: 'OPENAI_API_KEY не налаштовано на сервері' }, { status: 500 });
    ...
    if (!upstream.ok || !upstream.body) {
      ...
      return NextResponse.json({ error: 'Помилка провайдера ШІ: ' + detail.slice(0, 500) }, { status: 502 });
    }
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(streamController) {
        ...
        try {
          ...
          console.error('AI-проксі: помилка стрімінгу:', error);
          streamController.error(error instanceof Error ? error : new Error('Помилка стрімінгу AI-проксі'));
        }
      },
    });
    return new Response(stream, { headers: { 'Content-Type': 'text/event-stream; charset=utf-8', 'Cache-Control': 'no-cache, no-transform', Connection: 'keep-alive', 'X-Accel-Buffering': 'no' } });
  } catch (error) {
    console.error('AI-проксі: внутрішня помилка:', error);
    return NextResponse.json({ error: 'Внутрішня помилка AI-проксі' }, { status: 500 });
  }
}
