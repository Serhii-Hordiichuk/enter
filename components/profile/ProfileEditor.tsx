...
/** Profile editing form: display name, bio, and avatar color. Stored only locally. */
...
export function ProfileEditor({ seed, onSaved }: ProfileEditorProps): React.JSX.Element {
  ...
  const [colorId, setColorId] = useState<string>(profile.colorId);
  ...
  return (
    <form onSubmit={save} className="flex flex-col gap-4">
      <div className="flex items-center gap-4">
        <Avatar seed={seed} name={displayName} colorId={colorId} size={64} />
        <div className="min-w-0">
          <p className="truncate text-sm font-medium text-gotoap-ink">{displayName.trim() || 'Anonymous peer'}</p>
          <p className="truncate text-xs text-gotoap-ink-muted">{bio.trim() || 'No bio yet'}</p>
        </div>
      </div>
      ...
      <label className="block">
        ...
        <textarea
          ...
          className="w-full rounded-xl border border-gotoap-hover bg-gotoap-panel px-3 py-2 text-sm text-gotoap-ink placeholder:text-gotoap-ink-faint focus:border-gotoap-accent focus:outline-none focus:ring-2 focus:ring-gotoap-accent/30"
        />
      </label>
      <div>
          ...
          {AVATAR_COLOR_IDS.map((id) => {
            ...
            return (
              <button
                key={id}
                type="button"
                aria-label={'Avatar color ' + id}
                aria-pressed={active}
                onClick={() => setColorId(id)}
                className={'h-8 w-8 rounded-full transition ' + (active ? 'ring-2 ring-gotoap-accent ring-offset-2 ring-offset-gotoap-panel' : 'hover:scale-110')}
              >
